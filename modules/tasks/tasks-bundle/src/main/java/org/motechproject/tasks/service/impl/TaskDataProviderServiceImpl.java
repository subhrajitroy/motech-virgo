...
public class TaskDataProviderServiceImpl implements TaskDataProviderService {
    ...
    public TaskDataProvider registerProvider(final InputStream stream) {
        ...
        if (update) {
            ...
            parameters.put(EventDataKeys.DATA_PROVIDER_NAME, provider.getName());

            eventRelay.sendEventMessage(new MotechEvent(EventSubjects.DATA_PROVIDER_UPDATE_SUBJECT, parameters));
        }
