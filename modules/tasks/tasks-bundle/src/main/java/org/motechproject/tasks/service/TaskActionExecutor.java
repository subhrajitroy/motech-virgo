...
class TaskActionExecutor {
    ...
    void execute(Task task, TaskActionInformation actionInformation, TaskContext taskContext) throws TaskHandlerException {
        ...
        if (!action.hasSubject()) {
            throw new TaskHandlerException(TaskFailureCause.ACTION, "task.error.cantExecuteAction");
        }
    ...
    private ActionEvent getActionEvent(TaskActionInformation actionInformation)
        ...
        try {
            ...
            throw new TaskHandlerException(TaskFailureCause.TRIGGER, "task.error.actionNotFound", e);
        }
            ...
            if (info.getValues().containsKey(key)) {
                ...
                if (template == null) {
                    throw new TaskHandlerException(
                        TaskFailureCause.TRIGGER, "task.error.templateNull", key, action.getDisplayName()
                    );
                }

                switch (actionParameter.getType()) {
                    case LIST:
                        parameters.put(key, convertToList((List<String>) ParameterType.LIST.parse(template)));
                        break;
                    case MAP:
                    ...
                    default:
                        try {
                            ...
                            throw new TaskHandlerException(TaskFailureCause.TRIGGER, ex.getMessage(), ex, key);
                        }
                }
            } else {
                if (actionParameter.isRequired()) {
                    throw new TaskHandlerException(
                        TaskFailureCause.TRIGGER, "task.error.taskActionNotContainsField",
                        action.getDisplayName(), key
                    );
                } else if(actionParameter.getType() == ParameterType.MAP) {
                    parameters.put(key, new HashMap<>());
                } else if(actionParameter.getType() == ParameterType.LIST) {
        ...
        if (serviceAvailable) {
            ...
            try {
                ...
                try {
                    ...
                    throw new TaskHandlerException(
                            TaskFailureCause.ACTION, "task.error.serviceMethodInvokeError", e,
                            serviceMethod, action.getServiceInterface()
                    );
                }
            } catch (NoSuchMethodException e) {
                throw new TaskHandlerException(
                        TaskFailureCause.ACTION, "task.error.notFoundMethodForService", e,
                        serviceMethod, action.getServiceInterface()
                );
            }
        }
