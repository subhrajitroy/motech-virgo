...
public class TaskTriggerHandler implements TriggerHandler {
    ...
    private void handleError(MotechEvent trigger, Task task, TaskHandlerException e) {
        ...
        errorParam.put(EventDataKeys.TASK_FAIL_MESSAGE, e.getMessage());
        errorParam.put(EventDataKeys.TASK_FAIL_STACK_TRACE, ExceptionUtils.getStackTrace(e));
        errorParam.put(EventDataKeys.TASK_FAIL_FAILURE_DATE, DateTime.now());
        errorParam.put(EventDataKeys.TASK_FAIL_FAILURE_NUMBER, failureNumber);
        errorParam.put(EventDataKeys.TASK_FAIL_TRIGGER_DISABLED, task.isEnabled());
        errorParam.put(EventDataKeys.TASK_FAIL_TASK_ID, task.getId());
        errorParam.put(EventDataKeys.TASK_FAIL_TASK_NAME, task.getName());

        Map<String, Object> param = trigger.getParameters();
        param.put(EventDataKeys.HANDLER_ERROR_PARAM, errorParam);
